...
## Wiederkehrende Inhaltsblöcke = Datengetrieben über `content.json`
- **`content.json` ist die Single Source of Truth** für Blöcke, die auf mehreren Seiten
  identisch vorkommen – aktuell der **`berater-band`** („Beratung und Kontakt": zwei
...
- **Workflow bei JEDER Textänderung (eigenständig, ohne Rückfrage):**
  ...
  2. Dann **alle** statischen Vorkommen im HTML nachziehen
     (`index.html` #beratung, `oval-saunen.html`, `fasssaunen.html`, `vorraumvarianten.html`).
...
Breadcrumb → `sec-head` → `.ov-grid` mit `.ov-card`-Kacheln → `berater-band`
